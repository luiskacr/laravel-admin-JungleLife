    ...
    <div class="card">
        ...
        <div class="card-body ">
            <form class="row g-3 fv-plugins-bootstrap5 fv-plugins-framework" action="{{ route('tours.update',$tour->id) }}" method="post" >
                        ...
                        <select id="time"  class="form-select" name="time" >
                            ...
                            @foreach($timetables as $timetable)
                                ...
                                @if($start == $timetableStart and $end == $timetableEnd)
                                    <option value="{{ $timetable->id }}" selected>{{ __('app.from'). Carbon\Carbon::parse($timetable->start)->format('g:i A'). __('app.to') . Carbon\Carbon::parse($timetable->end)->format('g:i A')  }}</option>
                                @else
                ...
                <div class="col-12">
                    <div class="col-md-6 fv-plugins-icon-container fv-plugins-bootstrap5-row-invalid">
                        <label class="form-label" for="tour-type">{{__('app.tour_type_singular')}}</label>
                        <select id="tour-type" class="form-select" name="tour-type">
                            @foreach($tourTypes as $tourType)
                                @if($tourType->id == $tour->type)
