                ...
                <div class="tab-pane fade" id="navs-top-messages" role="tabpanel">
                    <div class="row">
                        ...
                        <div class="col-12 mt-5">
                            ...
                            <div class="float-end">
                                <button class="btn btn-primary" type="button"  onclick="showSearchClientModal({{ json_encode($tour->id) }})" {{ $tour->state != 1 ? 'disabled' : '' }}>
                                    {{ __('app.search_client') }}
                                </button>
                                <button class="btn btn-primary" type="button"  onclick="showCreateClientModal()" {{ $tour->state != 1 ? 'disabled' : '' }}>
                                    {{ __('app.create') }}
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="card-datatable  table-responsive">
                            <table id="table" class="datatables-basic table border-top dataTable no-footer dtr-column">
                                ...
                                @foreach($clients as $client)
                                    <tr>
                                        <th>{{ $client->getClient->id }}</th>
                                        <th>{{ $client->getClient->name }}</th>
                                        <th>{{ $client->getClient->email }}</th>
                                        <th>{{ $client->getClient->clientTypes->name }}</th>
                                        <th>
                                            <div class="justify-content-between">
                                                <a class="m-2" href="{{ route('clients.show',$client->getClient->id) }}"><i class="bx bxs-show me-1"></i> {{ __('app.crud_show') }}</a>
